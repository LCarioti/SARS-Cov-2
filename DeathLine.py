...
conf = pd.read_csv('Ecdc-TimeSeries_death.csv')
...
dfIS.to_csv('Top20-Death.csv',index=False)
...
dfPCT.reset_index().to_csv('Top20DayPerc-Death.csv',index=False)

dfDIFF = dfISI.diff().fillna(0)
dfDIFF[ dfDIFF < 0] = 0

dfDIFF.reset_index().to_csv('Top20Daily-Death.csv',index=False)
