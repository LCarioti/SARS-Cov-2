...
Conf <- read.csv('Top20-Death.csv', check.names=FALSE)
...
ggsave('Top20Daily-Death.png',dpi=600, width= 37, height=22, units='cm')
...
ggplot(long2, aes(x=index, y=value,fill=variable)) + geom_bar(position = position_stack(reverse = TRUE) , stat="identity", colour='black') + scale_fill_manual(values = DcolorR) + xlab('Date') + ylab('Daily Death') + theme_bw() + theme(axis.text.x=element_text(color="red", angle=90, size=10, vjust=0.7, hjust=0.7  ), axis.text.y=element_text(color="red", angle=0, size=10, vjust=0.5  ), axis.title.x = element_text(colour='red', size=15), axis.title.y = element_text(colour='red', size=15)  , plot.background = element_rect(fill = "white") , strip.background = element_rect(fill="white") , strip.text = element_text(colour='red', size=20), legend.title=element_blank(), legend.text = element_text(colour="red", size=10)  ) + scale_x_discrete(breaks=c('2020-01-22', '2020-02-05',  '2020-02-19',  '2020-03-04','2020-03-18','2020-04-01', '2020-04-15', '2020-04-29','2020-05-13','2020-05-27','2020-06-10','2020-06-24','2020-07-08' ))
ggsave('Top20DailyBar-Death.png',dpi=600, width= 37, height=22, units='cm')


############# PERC 

perc <- read.csv('Top20DayPerc-Death.csv', check.names=FALSE)
...
ggplot(long3, aes(x=index, y=value, fill=variable)) + geom_bar(position = position_stack(reverse = TRUE) , stat="identity",colour='black') +  scale_fill_manual(values = DcolorR) +   facet_wrap( ~ variable, scales='free_y', nrow=5, ncol=4) + xlab('Date') + ylab('Daily Increase %') + theme_dark()  + theme(axis.text.x=element_text(color="white", angle=90, size=10, vjust=0.7, hjust=0.7  ), axis.text.y=element_text(color="white", angle=0, size=10, vjust=0.5  ), axis.title.x = element_text(colour='white', size=15), axis.title.y = element_text(colour='white', size=15)  , plot.background = element_rect(fill = "black") , strip.background = element_rect(fill="black") , strip.text = element_text(colour='white', size=20)  , legend.position="none"  ) + scale_x_discrete(breaks=c('2020-01-22', '2020-02-05',  '2020-02-19',  '2020-03-04','2020-03-18','2020-04-01', '2020-04-15', '2020-04-29','2020-05-13','2020-05-27','2020-06-10','2020-06-24','2020-07-08' )) + scale_y_continuous(trans="log1p", breaks = c(1,10,100,1000),limits=c(0,1000)   )
ggsave('Top20DailyPerc-Death.png',dpi=600, width= 37, height=22, units='cm')
